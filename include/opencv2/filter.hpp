...
namespace impl {
	...
	void debugFilter(cv::InputArray original, cv::InputArray result, const CallMetaData& data,
...
{
	if(debugMode()) {
		impl::debugFilter(original, result, metaData, description, view);
	}
}
static inline void debugFilter(cv::InputArray original, cv::InputArray result,
		impl::CallMetaData metaData, const ::std::string& description,
		const ::std::string& view = "")
{
	if(debugMode()) {
		impl::debugFilter(original, result, metaData, description.c_str(), view.c_str());
	}
}
#else
