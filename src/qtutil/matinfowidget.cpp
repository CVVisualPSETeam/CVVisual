...
MatInfoWidget::MatInfoWidget(cv::Mat mat, QWidget *parent) : QWidget{ parent }
{
	...
	connect(zoomSpin, SIGNAL(valueChanged(double)), this,
	        SIGNAL(getZoom(qreal)));
	updateMat(mat);
	TRACEPOINT;
}
...
void MatInfoWidget::updateConvertStatus(ImageConversionResult result)
{
	...
	case ImageConversionResult::FLOAT_OUT_OF_0_TO_1:
		labelConvert->setText(
		    qs.append("Float values out of range [0,1]"));
		break;
	case ImageConversionResult::NUMBER_OF_CHANNELS_NOT_SUPPORTED:
		labelConvert->setText(
		    qs.append("Unsupported number of channels"));
		break;
	...
	default:
		labelConvert->setText(
		    qs.append("Unknown result from convert function"));
		break;
	}
	TRACEPOINT;
}
...
void MatInfoWidget::updateMat(cv::Mat mat)
{
	...
	else
	{
		...
		{
		case CV_8U:
			labelType->setText("Type: CV_8U");
			break;
		case CV_8S:
			labelType->setText("Type: CV_5U");
			break;
		case CV_16U:
			labelType->setText("Type: CV_16U");
			break;
		case CV_16S:
			labelType->setText("Type: CV_16S");
			break;
		case CV_32S:
			labelType->setText("Type: CV_32S");
			break;
		case CV_32F:
			labelType->setText("Type: CV_32F");
			break;
		case CV_64F:
			labelType->setText("Type: CV_64S");
			break;
		default:
