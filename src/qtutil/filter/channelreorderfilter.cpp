	...
	channelAssignment_{}
{
	...
	layout_->addWidget(util::make_unique<QLabel>("Assignment for the old channels").release());
...
void ChannelReorderFilter::applyFilter(InputArray in,OutputArray out) const
{
	...
	cv::Mat zeros=cv::Mat::zeros(chans.front().rows,
					chans.front().cols,
					chans.front().type());
	std::vector<cv::Mat> toMerge{};
	for(std::size_t i=0;i<channelAssignment_.size();i++)
	{
		if( static_cast<std::size_t>(channelAssignment_.at(i)->value())<chans.size())
		{
			TRACEPOINT;
			toMerge.push_back(chans.at(channelAssignment_.at(i)->value()));
		}else{
			TRACEPOINT;
