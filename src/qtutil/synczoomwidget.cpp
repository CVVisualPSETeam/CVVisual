...
cvv::qtutil::SyncZoomWidget::SyncZoomWidget(std::vector<cvv::qtutil::ZoomableImage *> images, QWidget *parent):
	QWidget(parent),
	images_{images},
	currentIdx_{images_.size()},
	buttonGroup_{new QButtonGroup}
{
	if(images_.size()>=2){
		...
		for(size_t i=0;i<images_.size();i++){
			...
			auto checkbox=util::make_unique<QCheckBox>(QString{"Image Nr. %1" }.arg(i));
...
void SyncZoomWidget::selectMaster(int id)
{
	if(currentIdx_<images_.size()){
		disconnect(images_.at(currentIdx_),SIGNAL(updateArea(QRectF,qreal)),
			this,SIGNAL(updateArea(QRectF,qreal)));
	...
	if(currentIdx_<images_.size()){
		...
		connect(images_.at(currentIdx_),SIGNAL(updateArea(QRectF,qreal)),
			this,SIGNAL(updateArea(QRectF,qreal)));
	}
}
