...
RawviewTableRow::RawviewTableRow(cv::DMatch match, cv::KeyPoint keyPoint1, cv::KeyPoint keyPoint2):
	match{match}, keyPoint1{keyPoint1}, keyPoint2{keyPoint2}, hasLonelyKeyPoint_{false}
{
	TRACEPOINT;
}
									...
									bool singleKeyPointRows)
{
	...
	if (format == "CSV")
	{
		...
		{
			header << "x" << "y" << "size" << "angle" << "response" 
				   << "octave" << "class id";
		}
		else
		{
	...
	for (auto &row : rows)
	{
				 ...
				 && !singleKeyPointRows)
		{
			...
			lines << QString("{%1, %2, %3}").arg(
						 combinestr("match", QString("{%1}").arg(match.join(", "))),
						 combinestr("key point 1", QString("{%1}").arg(keyPoint1.join(", "))),
						 combinestr("key point 2", QString("{%1}").arg(keyPoint2.join(", "))));
			}
	if (format == "CSV" && singleKeyPointRows)
	{
		QStringList line;
		line << floatstr(row.keyPoint1XCoord()) << floatstr(row.keyPoint1YCoord())
			 << floatstr(row.keyPoint1Size()) << floatstr(row.keyPoint1Angle())
			 << floatstr(row.keyPoint1Response()) << intstr(row.keyPoint1Octave())
			 << floatstr(row.keyPoint1ClassId()) << intstr(row.isLeftSingleKeyPoint() ? 1 : 2);
		lines << line.join(",");
	}
	else if ((format == "JSON" || format == "PYTHON" || format == "RUBY") 
			 && singleKeyPointRows)
	{
		...
		keyPoint << combinestr("x", floatstr(row.keyPoint1XCoord())) 
			 << combinestr("y", floatstr(row.keyPoint1YCoord()))
			 << combinestr("size", floatstr(row.keyPoint1Size()))
			 << combinestr("angle", floatstr(row.keyPoint1Angle())) 
			 << combinestr("response", floatstr(row.keyPoint1Response())) 
			 << combinestr("octave", intstr(row.keyPoint1Octave()))
			 << combinestr("class id", floatstr(row.keyPoint1ClassId()))
			 << combinestr("img number", intstr(row.isLeftSingleKeyPoint() ? 1 : 2));
										...
										const std::vector<cv::DMatch>& matches)
{
	...
	for (auto &match : matches)
	{
		...
		matchRowList.append(RawviewTableRow(match, keyPoints1.at(match.queryIdx), keyPoints2.at(match.trainIdx)));
	}
	for (size_t i = 0; i < usedKeyPoints1.size(); i++)
	{
		if (usedKeyPoints1.find(i) != usedKeyPoints1.end())
		{
			singleRowList.push_back(RawviewTableRow(keyPoints1.at(i), true));
		}
	}
	for (size_t i = 0; i < usedKeyPoints2.size(); i++)
	{
		...
		{
			singleRowList.push_back(RawviewTableRow(keyPoints2.at(i), true));
		}
	}
	TRACEPOINT;
