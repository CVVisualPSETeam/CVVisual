...
class MultiViewCallTab:
		public CallTab, public cvv::qtutil::RegisterHelper<ViewType, const CallType&, QWidget*>
{
public:
	...
	MultiViewCallTab(const QString& tabName, const CallType& call):
		call_{call},
		currentIndexChanged{[&]() {	vlayout_->removeWidget(view_);
						view_->setVisible(false);
						setView(); }},
		helpButtonClicked{[&]() {	qtutil::openHelpBrowser(viewId_); }},
		setAsDefaultButtonClicked{[&]() { qtutil::setSetting(default_scope_, default_key_, viewId_); }}
	{
		TRACEPOINT;
		setName(tabName);
		TRACEPOINT;
	 ...
	 * View needs to offer a constructor of the form View(const cvv::impl::CallType&, QWidget*).
	 ...
	 */
	template<class View>
	static bool registerView(const QString& name)
	{
		TRACEPOINT;
		return MultiViewCallTab<ViewType, CallType>::registerElement(name,
			[](const CallType& call, QWidget* parent){
				TRACEPOINT;
...
protected:
	...
	void createGui()
	{
		...
		if(!this->select(viewId_))
		{
			...
			/* If viewId_ does not name a valid View, it will be attempted to set standard_default_.
		...
		QObject::connect(setAsDefaultButton_, SIGNAL(clicked()), &setAsDefaultButtonClicked, SLOT(slot()));
		QObject::connect(helpButton_, SIGNAL(clicked()), &helpButtonClicked, SLOT(slot()));
...
private:
	...
	void setView()
	{
		...
		{
			viewHistory_.emplace(this->selection(), ((*this)()(*call_, this).release()));
			view_ = viewHistory_.at(this->selection());
			vlayout_->addWidget(view_);
		}
		TRACEPOINT;
