	...
	parent{parent}, group{std::move(group)}
{
    ...
    connect(qTable, SIGNAL(itemSelectionChanged()), this, SLOT(selectionChanged()));
	...
	{
		list << "x" << "y" << "size" << "angle" << "response" << "octave" << "class id" << "img number";		
	}
	else
	{
	...
	if (currentRowIndexes.size() > 0)
	{
		...
		if (text == "Show selected rows in view")
		{
			if (single)
			{
				...
				parent->getParent()->keyPointsSelected(keyPoints);
			}
			else
			{
		...
		else
		{
			...
			for (auto format : formats)
			{
				if (text == QString("Copy as %1").arg(format))
				{
					...
					formattedRows = RawviewTableRow::rowsToText(rows, format, single);
					DEBUG(formattedRows);
