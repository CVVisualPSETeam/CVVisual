...
/**
 * @brief A window showing the raw view, that displays the raw data of a match call.
 */
...
public:
	 ...
	 */
    RawviewWindow(QString title, const std::vector<cv::KeyPoint>& keypoints1,
			const std::vector<cv::KeyPoint>& keypoints2,
			const std::vector<cv::DMatch>& matches);
	
	/**
	 * @brief Constructor of this class.
	 * A view will be created when the matchesSelected gets called the first time.
	 * @param title window title
	 * @param keypoints1 left keypoints
	 * @param keypoints2 right keypoints
	 */
