...
#else
#define DEBUG(...)                                                             \
	::cvv::dbg::log({ __FILE__, __PRETTY_FUNCTION__, __LINE__ },           \
	                __VA_ARGS__)
#define DEBUGF(...)                                                            \
	::cvv::dbg::logF({ __FILE__, __PRETTY_FUNCTION__, __LINE__ },          \
	                 __VA_ARGS__)
#define TRACEPOINT                                                             \
	::cvv::dbg::log({ __FILE__, __PRETTY_FUNCTION__, __LINE__ },           \
	                "tracepoint")
...
template <typename... T> void log(const Location &loc, const T &... args)
{
	if (getLoggingState())
	{
