...
constexpr PrintableCategory getPrintableCategory() {
	return
		...
		isQString<T>()    ? PrintableCategory::QString     :
...
template<typename T> void printToStreamTagged(std::ostream& stream, const T&, QStringTag);
...
template<typename T> constexpr bool isQString() { return IsQStringHelper<T>::value; }
...
// QString
template<typename T> void printToStreamTagged(std::ostream& stream, const T& arg, QStringTag) {
